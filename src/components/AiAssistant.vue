...
onMounted(() => {
  ...
  if (size) gifSize.value = Number(size) || 100

  const saved = localStorage.getItem('ai_assistant_pos')
  if (saved) {
    try {
      const p = JSON.parse(saved)
      btnX.value = p.btnX ?? window.innerWidth - 120
      btnY.value = p.btnY ?? window.innerHeight - 120
    ...
    btnX.value = 30
    btnY.value = window.innerHeight - 240
    dlgX.value = 30
