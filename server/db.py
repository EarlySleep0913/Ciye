...
CREATE TABLE IF NOT EXISTS progress_snapshots (
    ...
    session_date TEXT,
    session_book INTEGER,
    before_studied_ids TEXT,
    created_at TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_snapshots_user_event ON progress_snapshots(user_id, event_id);
...
def init_db() -> None:
    ...
    # Migrations: keep older app.db files compatible with the optimized scheduler.
    progress_columns = {
        row["name"] for row in conn.execute("PRAGMA table_info(progress)").fetchall()
    }
    progress_migrations = {
        "memory_strength": "ALTER TABLE progress ADD COLUMN memory_strength REAL NOT NULL DEFAULT 1.0",
        "review_count": "ALTER TABLE progress ADD COLUMN review_count INTEGER NOT NULL DEFAULT 0",
        "lapse_count": "ALTER TABLE progress ADD COLUMN lapse_count INTEGER NOT NULL DEFAULT 0",
        "difficulty": "ALTER TABLE progress ADD COLUMN difficulty REAL NOT NULL DEFAULT 5.0",
        "first_seen_at": "ALTER TABLE progress ADD COLUMN first_seen_at TEXT",
        "last_grade": "ALTER TABLE progress ADD COLUMN last_grade TEXT",
    }
    for column, sql in progress_migrations.items():
        if column not in progress_columns:
            conn.execute(sql)
            conn.commit()
    conn.execute(
        "UPDATE progress SET review_count = attempts WHERE review_count = 0 AND attempts > 0"
    )
    conn.execute(
        "UPDATE progress SET first_seen_at = last_seen WHERE first_seen_at IS NULL AND last_seen IS NOT NULL"
    )
