...
from .progress_service import VALID_ACTIONS, ensure_progress_for_book, submit_feedback, undo_last_feedback
from .scheduler import build_today_session, review_forecast
    ...
    def do_GET(self) -> None:
      try:
        ...
        routes = {
            ...
            "/api/review-forecast": lambda: self._review_forecast(uid),
    ...
    def do_POST(self) -> None:
      try:
        ...
        routes = {
            ...
            "/api/progress/undo": lambda: self._progress_undo(uid),
...
✅ 鼓励学生 - 多用鼓励性语言"""
    ...
    def _today(self, uid: int) -> None:
        session_data = build_today_session(uid)
        word_ids = session_data["word_ids"]
        studied_ids = session_data["studied_ids"]
        limit = session_data["daily_new_limit"]
        aid = session_data["active_book_id"]
        conn = get_conn()
        today_str = today()
        ...
        if not word_ids:
            _json_response(self, {
                ...
                "reviews": [], "new_words": [],
                "review_load": session_data["review_load"],
            })
            return
        ...
        _bg_enrich_images([wid for wid in word_ids if wid in row_map])
        _json_response(self, {
            "daily_new_limit": limit, "active_book_id": aid,
            "review_load": session_data["review_load"],
    ...
    def _review_forecast(self, uid: int) -> None:
        _json_response(self, {"days": review_forecast(uid, 7)})
    ...
    def _wrong_words(self, uid: int) -> None:
        rows = get_conn().execute(
            """SELECT w.id, w.word, w.translation, w.phonetic,
                      ...
                      p.lapse_count, p.due_date, p.last_grade
    ...
    def _test_words(self, uid: int, query: dict) -> None:
        ...
        if range_type == "today":
            # Only words actually studied today, not every pending word in the queue.
            today_str = today()
            ...
            if session:
                word_ids = json.loads(session["studied_ids"])
            else:
                word_ids = []
            if word_ids:
                placeholders = ",".join("?" for _ in word_ids)
    ...
    def _test_check(self, uid: int) -> None:
        ...
        result = submit_feedback(
            uid,
            word_id,
            "known" if is_correct else "forgot",
            mark_studied=False,
        )
    ...
    def _progress(self, uid: int) -> None:
        ...
        try:
            result = submit_feedback(uid, word_id, action, mark_studied=True)
        except ValueError as exc:
            return _json_response(self, {"error": str(exc)}, 400)
        _json_response(self, result)

    def _progress_undo(self, uid: int) -> None:
        try:
            result = undo_last_feedback(uid)
        except ValueError as exc:
            return _json_response(self, {"error": str(exc)}, 400)
        _json_response(self, result)
    ...
    def _reset_today(self, uid: int) -> None:
        ...
        sessions = conn.execute(
            "SELECT word_ids FROM daily_session WHERE user_id = ? AND date = ?",
            (uid, today_str),
        ).fetchall()
        session_ids = []
        for session in sessions:
            session_ids.extend(json.loads(session["word_ids"]))
        ...
        if session_ids:
            ...
            conn.execute(
                ...
                [today_str, uid] + session_ids + [f"{today_str}%"],
            )
    ...
    def _reset_book_progress(self, uid: int) -> None:
        ...
        conn = get_conn()
        conn.execute(
            """UPDATE progress SET status = 'new', familiarity = 0, memory_strength = 1.0,
               review_count = 0, lapse_count = 0, difficulty = 5.0,
               attempts = 0, correct = 0, last_seen = NULL, due_date = ?,
               first_seen_at = NULL, last_grade = NULL, is_wrong = 0
