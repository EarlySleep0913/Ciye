...
def update_memory_strength(old_strength: float, action: str) -> float:
    """根据学习反馈更新记忆强度"""
    if action in ("known", "easy"):
        new_strength = old_strength + 1.0
    elif action == "vague":
        new_strength = old_strength + 0.3
    else:  # forgot
        new_strength = max(0.5, old_strength - 0.5)
    return round(min(max(new_strength, 0.5), 20.0), 2)
...
def get_retention_data(uid: int) -> dict:
    ...
    rows = conn.execute(
        f"""SELECT p.status, p.memory_strength, p.last_seen, p.familiarity,
                   p.attempts, p.correct, p.is_wrong
            FROM progress p
...
def get_word_retention(uid: int, word_id: int) -> dict:
    ...
    return {
        ...
        "next_review": calc_next_review(s, last),
        "at_risk": current_r < RETENTION_THRESHOLD,
...
def get_review_queue_ebbinghaus(uid: int) -> list[dict]:
    ...
    rows = conn.execute(
        f"""SELECT w.id, w.word, w.translation, p.memory_strength, p.last_seen
    ...
    # 按保持率排序（最低的优先复习）
    due_words.sort(key=lambda x: x["retention"])
    return due_words
